/**
  ******************************************************************************
  * @file    b_u585i_iot02a_eeprom.c
  * @author  MCD Application Team
  * @brief   This file provides a set of functions needed to manage an I2C M24256
  *          EEPROM memory mounted on the B_U585I_IOT02A board(MB1551).
  ******************************************************************************
  * @attention
  *
  ...
  *           GPIO) are defined in b_u585i_iot02a_bus.h file, and the initialization is
  ...
  *                 an EEPROM instance. On the B_U585I_IOT02A board, there's one instance. Therefore,
...
/** @defgroup B_U585I_IOT02A_EEPROM EEPROM
  * @{
  */
...
int32_t BSP_EEPROM_WritePage(const uint32_t Instance, uint8_t *pBuffer, uint32_t PageNbr)
{
  ...
  else
  {
    ...
    the M24256 specification to ensure that the previous EEPROM access is finished */
    HAL_Delay(5);
