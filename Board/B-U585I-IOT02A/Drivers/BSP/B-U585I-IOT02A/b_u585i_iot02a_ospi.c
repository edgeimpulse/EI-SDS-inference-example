  ...
  ******************************************************************************
  ...
  [..]
   (#) This driver is used to drive the MX25LM51245G Octal NOR and the APS6408 Octal PSRAM
       external memories mounted on B_U585I_IOT02A board.
   ...
   (#) APS6408 Octal PSRAM memory Initialization steps:
       (++) Initialize the Octal PSRAM external memory using the BSP_OSPI_RAM_Init() function. This
            function includes the MSP layer hardware resources initialization and the
            OSPI interface with the external memory.
   ...
   (#) APS6408L Octal PSRAM memory operations
       ...
       (++) The function BSP_OSPI_RAM_ReadID() returns the identifier of the memory
            (see the OSPI memory data sheet)
...
/** @defgroup B_U585I_IOT02A_OSPI OSPI
  * @{
  */
...
int32_t BSP_OSPI_NOR_RegisterMspCallbacks(uint32_t Instance, BSP_OSPI_Cb_t *CallBacks)
{
  ...
  else
  {
    ...
    else if (HAL_OSPI_RegisterCallback(&hospi_nor[Instance],
                                       HAL_OSPI_MSP_DEINIT_CB_ID, CallBacks->pMspDeInitCb) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
/** @addtogroup B_U585I_IOT02A_OSPI_RAM_Private_Functions
  * @{
  */
/**
  * @brief  Initializes the OSPI MSP.
